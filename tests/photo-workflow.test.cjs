...
global.fetch = async (url, init) => {
  ...
  if (url.endsWith('/responses')) {
    ...
    const result = { summary: 'სატესტო პროდუქტი', warnings: ['შეამოწმეთ ქართული წარწერა'], outputs: [
      {role:'main', title:'დახურული ხედი', reason:'ფორმა', baseReferenceId:analyzedReferences[0], supportingReferenceIds:[analyzedReferences[1]], instruction:'Preserve text'},
      {role:'informative', title:'შიდა ნაწილი', reason:'კონსტრუქცია', baseReferenceId:analyzedReferences[1], supportingReferenceIds:[], instruction:'Preserve pockets'},
    ] };
...
test('photo workflow, URL migration and publication boundaries', async t=>{
  ...
  await t.test('generation uses only selected references and never publishes; failures retain drafts',async()=>{
    ...
    assert.equal(editInputs.length,2);assert.equal(editInputs[0].name,workflow.outputs[0].baseReferenceId+'.png');
