...
export default async function ProductPage({ params }: Props) {
  ...
  return <div className="min-h-screen bg-[#fcfcfc] pt-14 text-[#1a1b1c]">
    <JsonLd data={breadcrumbJsonLd([{ name: lang === "en" ? "Catalog" : "კატალოგი", path: `/${lang}/catalog` }, { name: typeName, path: typePath(lang, category) }, { name: productCode, path: productPath(lang, category, product) }])} />
    ...
    <main className="mx-auto max-w-7xl px-4 py-12 lg:px-8 lg:py-20" data-ga-page="catalog_product" data-ga-view-id={product.code || product.slug} data-ga-view-name={name} data-ga-view-category={category.slug} data-ga-view-category2={group.slug}>
      <nav aria-label={lang === "en" ? "Breadcrumb" : "ნავიგაცია"} className="mb-8 flex flex-wrap gap-2 text-xs text-gray-500"><Link href={`/${lang}/catalog`}>{lang === "en" ? "Catalog" : "კატალოგი"}</Link><span>/</span><Link href={typePath(lang, category)}>{typeName}</Link><span>/</span><span aria-current="page">{productCode}</span></nav>
