  ...
  properties: {
    ...
    outputs: {
      type: "array", items: {
        ...
        properties: {
          role: { type: "string", enum: ["main", "informative", "detail", "additional"] }, title: text, reason: text,
...
export async function analyzeReferences(workflow: PhotoWorkflow, productName: string) {
  ...
  const planned: PlannedOutput[] = (
    parsed.outputs as Omit<PlannedOutput, "id">[]
  ).map((output) => ({
    ...output,
    id: randomUUID(),
  }));
  ...
  if (!planned.length) {
    throw new PhotoError(
      "ანალიზმა გამოსადეგი კადრების გეგმა ვერ შემოგთავაზა. ხელახლა სცადეთ.",
      502
    );
  }
  ...
  const mainIndexes = planned
    .map((output, index) => (output.role === "main" ? index : -1))
    .filter((index) => index >= 0);

  if (mainIndexes.length === 0) {
    planned[0].role = "main";
    warnings.push(
      "AI-მ მთავარი კადრი ცალკე არ მონიშნა; მთავარ კადრად პირველი შეთავაზება იქნა არჩეული."
    );
  }

  if (mainIndexes.length > 1) {
    for (const index of mainIndexes.slice(1)) {
      planned[index].role = "informative";
    }

    warnings.push(
      "AI-მ ერთზე მეტი მთავარი კადრი მონიშნა; მთავარ კადრად პირველი დარჩა."
    );
  }

  const outputs = validatePlan(planned, workflow.references);
...
export async function editProductPhoto(workflow: PhotoWorkflow, output: PhotoOutput, correction: string) {
  ...
  return {
    buffer: Buffer.from(base64, "base64"), provenance: {
