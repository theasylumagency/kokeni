...
export type PhotoOutput = {
  ...
  draft?: {
    filename: string;
    approved: boolean;
    provenance: NonNullable<ProductImage["provenance"]>;
  };
  attempts: number;
  error?: string;
};
...
export function validatePlan(
  ...
  if (!Array.isArray(value) || !value.length || value.length > 8) {
    throw new PhotoError("გეგმაში საჭიროა 1–8 კადრი.");
  }
  ...
  const outputs = value.map((item): PhotoOutput => {
    if (!item || typeof item !== "object") {
      throw new PhotoError("არასწორი გეგმა.");
    }
    ...
    if (
      ...
      throw new PhotoError("კადრის იდენტიფიკატორი არასწორია.");
    }

    ids.add(item.id);

    if (
      !PHOTO_ROLES.includes(item.role) ||
      typeof item.title !== "string" ||
      !item.title.trim() ||
      item.title.length > 200 ||
      !refs.has(item.baseReferenceId)
    ) {
      throw new PhotoError("შეავსეთ კადრის სახელი, როლი და ძირითადი ფოტო.");
    }

    if (
      !Array.isArray(item.supportingReferenceIds) ||
      item.supportingReferenceIds.length > 3 ||
      item.supportingReferenceIds.some(
        (id: unknown) =>
          typeof id !== "string" ||
          !refs.has(id) ||
          id === item.baseReferenceId
      ) ||
      new Set(item.supportingReferenceIds).size !==
      item.supportingReferenceIds.length
    ) {
      throw new PhotoError("აირჩიეთ მაქსიმუმ 3 განსხვავებული დამხმარე ფოტო.");
    }

    if (
      typeof item.instruction !== "string" ||
      item.instruction.length > 1500 ||
      typeof item.reason !== "string" ||
      item.reason.length > 2000
    ) {
      throw new PhotoError("კადრის ინსტრუქცია ზედმეტად გრძელია.");
    }
    ...
    return {
      ...
      reason: item.reason,
      baseReferenceId: item.baseReferenceId,
      supportingReferenceIds: item.supportingReferenceIds,
      instruction: item.instruction,
      attempts: 0,
    };
  });

  if (outputs.filter((output) => output.role === "main").length !== 1) {
    throw new PhotoError("გეგმას ზუსტად ერთი მთავარი კადრი უნდა ჰქონდეს.");
  }
