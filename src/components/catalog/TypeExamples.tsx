...
export type ExampleCard = { id: string; name: string; code: string; href: string; image?: string; specifications: CatalogAttribute[]; typeSlug?: string; sectorSlug?: string };
...
export default function TypeExamples({ examples, locale }: { examples: ExampleCard[]; locale: Locale }) {
  ...
  return <>
    ...
    {visible.length ? <div className={styles.examples}>{visible.map(example => <Link key={example.id} href={example.href} className={styles.example} data-ga-item-id={example.code} data-ga-item-name={example.name} data-ga-item-category={example.typeSlug} data-ga-item-category2={example.sectorSlug}>
