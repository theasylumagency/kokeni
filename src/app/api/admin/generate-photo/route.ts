...
export async function POST(request: Request) {
  try {
    ...
    const { slotKey, rawImageUrl, categoryName } = await request.json();

    if (!slotKey || !rawImageUrl || !categoryName) {
      return NextResponse.json(
    ...
    // Map public URL to local filesystem path
    // e.g. rawImageUrl = "/uploads/products/1234.webp"
    const relativePath = rawImageUrl.replace(/^\/uploads\//, "");
    if (!relativePath || relativePath === rawImageUrl) {
      return NextResponse.json({ error: "Invalid image URL" }, { status: 400 });
    }

    const imagePath = path.join(process.cwd(), "public", "uploads", relativePath);

    let imageBuffer: Buffer;
    try {
      imageBuffer = await fs.readFile(imagePath);
    } catch (e) {
      console.error("Failed to read image from disk:", e);
      return NextResponse.json({ error: "Source image not found on server." }, { status: 404 });
    }

    // For file types, usually webp since kokeni AdvancedImageUploader converts to webp
    ...
    const imageBase64 = extractImageBase64(response as unknown as GenerateContentResponseShape);
